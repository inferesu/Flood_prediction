...
# MODIFIED: Predict the change over 5 days
...
def prepare_features(df):
    ...
    # MODIFIED: Target Change shifted by 5 days
...
# ================= EVALUATION =================
print("\n--- Linear Regression Evaluation (5D Delta-aligned) ---")
...
out_df.to_csv("lr_predictions_5d.csv", index=False)
print("\nSaved predictions to 'lr_predictions_5d.csv'.")

# ================= PLOT =================
plt.figure(figsize=(14,6))
plt.plot(wl_true,label="Actual WL (5 Days Ahead)")
plt.plot(wl_pred,'--', alpha=0.8, label="Predicted WL (5 Days Ahead)")
plt.title("Linear Regression 5D Flood Prediction (Delta-aligned)")
plt.legend()
plt.grid(True)
plt.show()