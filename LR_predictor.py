...
# ================= CONFIG =================
LOOK_BACK_PERIOD = 14  # Synced to match LSTM, XGBoost, and SVR
...
def prepare_features(df):
    ...
    # Target Change - ALIGNED WITH ANFIS
...
def create_flat_sequences(X, y, look_back):
    ...
    for i in range(len(X) - look_back):
        # Flattening 2D temporal window into 1D array for Linear Regression
...
print("Train shape (Flattened):", X_train.shape)
print("Test shape (Flattened):", X_test.shape)
...
# ================= EVALUATION =================
print("\n--- Linear Regression Evaluation (Delta-aligned) ---")
...
plt.plot(wl_pred, linestyle="--", alpha=0.8, color='green', label="Predicted WL (Reconstructed)")
plt.title("Linear Regression Prediction of Water Level (Delta-aligned)")
plt.legend()
plt.grid(True)
plt.show()