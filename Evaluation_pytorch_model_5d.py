...
def prepare_features(df):
    ...
    # Target: Modified for 5 days ahead
...
def build_anfis(n_inputs, n_mfs):
    ...
    # Updated model name to match the 5D training script
...
def evaluate():
    ...
    target = config["target"] # Should be "target_change_5d" in your JSON
    ...
    print("\n--- ANFIS 5D Flood Model Performance ---")
    print(f"MSE   : {mse:.3f}")
    ...
    # Updated competitor file expectation for the 5D baseline
    COMPETITOR_FILE = "rnn_predictions_5d.csv"
    try:
        ...
        if p_val_mse > 0.05:
            print(
                "Result: The difference in MSE between ANFIS 5D and the competitor is NOT statistically significant (p > 0.05).")
        else:
    ...
    # Output file adjusted for 5D predictions
    out.to_csv("anfis_predictions_5d.csv", index=False)
    print("\nSaved to anfis_predictions_5d.csv")
    print(out.head())
...
def save_rules_to_csv(model, feature_names, num_mfs):
    ...
    # Output file adjusted for 5D rules
    pd.DataFrame(rows).to_csv("anfis_rule_base_5d.csv", index=False)
    print("\nFull rule base saved to anfis_rule_base_5d.csv")
