...
# --- Configuration ---
DATA_FILE            = 'nemunas_lampedziai_complex_data_2024.csv'   # ← changed
STATION_CODE_HYDRO   = 'lampedziu-vms'                              # ← changed
STATION_CODE_METEO_1 = 'kauno-ams'                                  # ← changed
K_DECAY              = 0.85
...
def fetch_water_levels(station_code, start_date, end_date):
    ...
    if len(all_obs) == 0:
        print(f"   ❌ CRITICAL: No water level data — check station code '{station_code}'")
    elif len(all_obs) < 100:
        print(f"   ⚠️  WARNING: Only {len(all_obs)} records — suspiciously low")
    else:
...
def fetch_meteo_data(station_code, start_date, end_date):
    ...
    if len(all_obs) == 0:
        print(f"   ❌ CRITICAL: No meteo data — check station code '{station_code}'")
    elif skipped > len(all_obs) * 0.2:
        print(f"   ⚠️  WARNING: {skipped} days skipped — more than 20% missing")
    else:
...
def validate_merged_df(df, expected_days):
    ...
    if len(df) < expected_days * 0.8:
        print(f"   ⚠️  Only {len(df)} rows — expected ~{expected_days}.")
        passed = False
    else:
    ...
    if wl_zeros > len(df) * 0.1:
        print(f"   ⚠️  {wl_zeros} rows have water_level_cm = 0")
    else:
    ...
    print(f"   ✅ All validation checks passed" if passed else f"   ⚠️  Some checks failed")
    return passed
...
def main():
    ...
    print("=" * 60)
    print("   🌊 NEMUNAS–LAMPĖDŽIAI DATA PIPELINE")                 # ← changed
    ...
    if not w_data:
        print("\n❌ ABORTED: No water level data.")
        return
    if not m1_data:
        print("\n❌ ABORTED: No meteo data.")
        return
    ...
    if missing_before > len(df_final) * 0.1:
        print(f"   ⚠️  WARNING: {missing_before} rows had no meteo match")
