...
# ================= EVALUATION =================
print("\n--- XGBoost Evaluation (5D Delta-aligned) ---")
...
out_df.to_csv("xgboost_predictions_5d.csv", index=False)
print("\nSaved predictions to 'xgboost_predictions_5d.csv'.")

# ================= PLOT =================
plt.figure(figsize=(14, 6))
plt.plot(wl_true, label="Actual WL (5 Days Ahead)")
plt.plot(wl_pred, linestyle="--", alpha=0.8, label="Predicted WL (5 Days Ahead)")
plt.title("XGBoost 5D Prediction of Water Level (Delta-aligned)")
plt.legend()
plt.grid(True)
plt.show()