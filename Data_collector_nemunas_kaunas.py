...
# --- Configuration ---
DATA_FILE = 'nemunas_kaunas_complex_data_2024.csv'
STATION_CODE_HYDRO = 'kauno-vms'
STATION_CODE_METEO_1 = 'kauno-ams'
K_DECAY = 0.85  # Standard decay coefficient for API (Eq. 9)


def fetch_water_levels(station_code, start_date, end_date):
    """Fetches water level data day by day using only the measured endpoint."""
    all_obs       = []
    current       = start_date
    request_count = 0
            ...
            if resp.status_code == 200:
                ...
                for obs in data:
                    ...
                    if val is not None:
                        ...
                        batch_count += 1
                print(
                    f"   💧 {date_str}  |  +{batch_count:4} records  |  Total: {len(all_obs)}"
    ...
    print(f"   {'─' * 55}")
    print(f"   ✅ Water level fetch complete — {len(all_obs)} observations ({request_count} requests)\n")
    return all_obs
...
def apply_hydrological_logic(df):
    ...
    df['precip_kauno-ams'] = df['precip_kauno-ams'].clip(lower=0)   # ← fix column name
    df['water_level_cm']   = df['water_level_cm'].clip(lower=0)

    df['Pt'] = df['precip_kauno-ams']                               # ← fix column name
    print(f"   ✔️  [Eq. 9]  Pt — mean={df['Pt'].mean():.2f}mm, max={df['Pt'].max():.2f}mm")
    ...
    print(f"   ✔️  [Eq. 9]  API_t — mean={df['API_t'].mean():.2f}, max={df['API_t'].max():.2f}")
    ...
    df['SMI_t'] = df['temp_kauno-ams'].apply(                       # ← fix column name
        lambda x: max(0, x * 2.5) if x > 0 else 0
    )
    print(f"   ✔️  [Eq. 17] SMI_t — {(df['SMI_t'] > 0).sum()} melt days, max={df['SMI_t'].max():.2f}")

    df['S_t'] = np.cos(2 * np.pi * pd.to_datetime(df['timestamp']).dt.dayofyear / 365)
    print(f"   ✔️  [Eq. 11] S_t — range=[{df['S_t'].min():.2f}, {df['S_t'].max():.2f}]")

    df['delta_WL_t'] = df['water_level_cm'].diff().fillna(0)
    print(f"   ✔️  [Eq. 21] delta_WL_t — rising={(df['delta_WL_t'] > 0).sum()}, falling={(df['delta_WL_t'] < 0).sum()}")

    print(f"   {'─' * 55}")
    print(f"   ✅ Hydrological logic applied\n")
    return df
...
def main():
    ...
    print("=" * 60)
    print("   🌊 DANIJA HYDROLOGICAL DATA PIPELINE")
