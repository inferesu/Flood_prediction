...
TRAIN_DATA_FILE = '../kanalas_lankupiu_complex_data_2024.csv'  # ← changed
FEATURES_LIST   = ['API_norm', 'S_t', 'SMI_t', 'Pt', 'delta_WL_t']
TARGET          = 'target_change_3d'                          # ← changed to 3d

MODEL_FILE    = '../ANFIS_models/kanalas_lankupiu_anfis_model_3d.pth'  # ← changed
SCALER_X_FILE = '../Scalers/kanalas_lankupiu_scaler_X_3d.pkl'  # ← changed
SCALER_Y_FILE = '../Scalers/kanalas_lankupiu_scaler_y_3d.pkl'  # ← changed
CONFIG_FILE   = '../Training_configs/kanalas_lankupiu_training_config_3d.json'  # ← changed


# ---------------------------------------------------------------------------
# Feature engineering  — ventes-ams + klaipedos-ams
# ---------------------------------------------------------------------------
def prepare_complex_features(df: pd.DataFrame) -> pd.DataFrame:
    df = df.copy().sort_index()

    df['Pt'] = df[['precip_ventes-ams', 'precip_klaipedos-ams']].mean(axis=1)    # ← changed
    ...
    avg_t = df[['temp_ventes-ams', 'temp_klaipedos-ams']].mean(axis=1)           # ← changed
    df['SMI_t'] = avg_t.apply(lambda x: max(0, x * 2.5) if x > 0 else 0)

    df['delta_WL_t']       = df['water_level_cm'].diff().fillna(0)
    df['target_change_3d'] = df['water_level_cm'].shift(-3) - df['water_level_cm']  # ← changed
...
def build_anfis(num_inputs: int, num_mfs: int) -> AnfisNet:
    ...
    return AnfisNet('Kanalas Lankupiu Flood Model 3D', invardefs, ['y'], hybrid=True)  # ← changed
...
def train_and_save():
    print("=" * 60)
    print("   🌊 KANALAS LANKUPIŲ ANFIS 3-DAY TRAINING PIPELINE")  # ← changed
    print(f"   Input  : {TRAIN_DATA_FILE}")
    print(f"   Model  : {MODEL_FILE}")
    print(f"   Stations: ventes-ams + klaipedos-ams")               # ← changed
    print(f"   LR     : {LR}")
    print("=" * 60)
