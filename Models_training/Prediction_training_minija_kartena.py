...
TRAIN_DATA_FILE = '../minija_kartena_complex_data_2024.csv'
FEATURES_LIST   = ['API_norm', 'S_t', 'SMI_t', 'Pt', 'delta_WL_t']
TARGET          = 'target_change'

# Output files — kartena prefix so they don't clash with the Priekulė models
MODEL_FILE    = '../ANFIS_models/kartena_anfis_model.pth'
SCALER_X_FILE = '../Scalers/kartena_scaler_X.pkl'
SCALER_Y_FILE = '../Scalers/kartena_scaler_y.pkl'
CONFIG_FILE   = '../Training_configs/kartena_training_config.json'
...
def prepare_complex_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Average precipitation across kretingos + vezaiciu
    df['Pt'] = df[['precip_kretingos-ams', 'precip_vezaiciu-ams']].mean(axis=1)

    # API_t = Pt + k * API_t-1  (Eq. 9)
    api_vals, curr_api = [], 0.0
    for p in df['Pt']:
    ...
    # S_t — seasonality cosine  (Eq. 11)
    d = pd.to_datetime(df['timestamp']).dt.dayofyear
    df['S_t'] = np.cos((2 * np.pi * d) / 365)

    # SMI_t — snowmelt index  (Eq. 17)
    avg_t = df[['temp_kretingos-ams', 'temp_vezaiciu-ams']].mean(axis=1)
    df['SMI_t'] = avg_t.apply(lambda x: max(0.0, x * 2.5) if x > 0 else 0.0)

    # delta_WL_t — trend persistence  (Eq. 21)
...
def build_anfis(num_inputs: int, num_mfs: int) -> AnfisNet:
    ...
    return AnfisNet('Kartena Flood Model', invardefs, ['y'], hybrid=True)
...
def train_and_save():
    print("=" * 60)
    print("   🌊 KARTENA (MINIJA) ANFIS 1-DAY TRAINING PIPELINE")
    print(f"   Input    : {TRAIN_DATA_FILE}")
    print(f"   Model    : {MODEL_FILE}")
    print(f"   Hydro    : kartenos-vms")
    print(f"   Meteo    : kretingos-ams + vezaiciu-ams")
    print(f"   LR       : {LR}")
    print("=" * 60)
