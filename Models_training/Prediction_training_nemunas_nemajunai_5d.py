...
TRAIN_DATA_FILE = '../nemunas_nemajunai_complex_data_2024.csv'  # ← changed
FEATURES_LIST   = ['API_norm', 'S_t', 'SMI_t', 'Pt', 'delta_WL_t']
TARGET          = 'target_change_5d'

MODEL_FILE    = '../ANFIS_models/nemunas_nemajunai_anfis_model_5d.pth'  # ← changed
SCALER_X_FILE = '../Scalers/nemunas_nemajunai_scaler_X_5d.pkl'  # ← changed
SCALER_Y_FILE = '../Scalers/nemunas_nemajunai_scaler_y_5d.pkl'  # ← changed
CONFIG_FILE   = '../Training_configs/nemunas_nemajunai_training_config_5d.json'  # ← changed


# ---------------------------------------------------------------------------
# Feature engineering  — birstono-ams + alytaus-ams
# ---------------------------------------------------------------------------
def prepare_complex_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df['precip_alytaus-ams']  = df['precip_alytaus-ams'].clip(lower=0)    # ← changed
    df['water_level_cm']      = df['water_level_cm'].clip(lower=0)

    df['Pt'] = df[['precip_birstono-ams', 'precip_alytaus-ams']].mean(axis=1)  # ← changed
    ...
    avg_t = df[['temp_birstono-ams', 'temp_alytaus-ams']].mean(axis=1)    # ← changed
...
def build_anfis(num_inputs: int, num_mfs: int) -> AnfisNet:
    ...
    return AnfisNet('Nemunas Nemajunai Flood Model 5D', invardefs, ['y'], hybrid=True)  # ← changed
...
def train_and_save():
    print("=" * 60)
    print("   🌊 NEMUNAS–NEMAJŪNAI ANFIS 5-DAY TRAINING PIPELINE")  # ← changed
    print(f"   Input  : {TRAIN_DATA_FILE}")
    print(f"   Model  : {MODEL_FILE}")
    print(f"   Stations: birstono-ams + alytaus-ams")                # ← changed
    print(f"   LR     : {LR}")
    print("=" * 60)
