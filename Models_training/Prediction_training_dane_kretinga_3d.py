...
TRAIN_DATA_FILE = '../dane_kretinga_complex_data_2024.csv'  # ← changed
FEATURES_LIST   = ['API_norm', 'S_t', 'SMI_t', 'Pt', 'delta_WL_t']
TARGET          = 'target_change_3d'

MODEL_FILE    = '../ANFIS_models/dane_kretinga_anfis_model_3d.pth'  # ← changed
SCALER_X_FILE = '../Scalers/dane_kretinga_scaler_X_3d.pkl'  # ← changed
SCALER_Y_FILE = '../Scalers/dane_kretinga_scaler_y_3d.pkl'  # ← changed
CONFIG_FILE   = '../Training_configs/dane_kretinga_training_config_3d.json'  # ← changed


# ---------------------------------------------------------------------------
# Feature engineering  — single station (kretingos-ams only)
# ---------------------------------------------------------------------------
def prepare_complex_features(df: pd.DataFrame) -> pd.DataFrame:
    df = df.copy().sort_index()

    df['Pt'] = df['precip_kretingos-ams']                     # ← changed
    ...
    df['SMI_t'] = df['temp_kretingos-ams'].apply(             # ← changed
...
def build_anfis(num_inputs: int, num_mfs: int) -> AnfisNet:
    ...
    return AnfisNet('Dane Kretinga Flood Model 3D', invardefs, ['y'], hybrid=True)  # ← changed
...
def train_and_save():
    print("=" * 60)
    print("   🌊 DANĖ–KRETINGA ANFIS 3-DAY TRAINING PIPELINE")   # ← changed
    print(f"   Input  : {TRAIN_DATA_FILE}")
    print(f"   Model  : {MODEL_FILE}")
    print(f"   Station: kretingos-ams only")                      # ← changed
    print(f"   LR     : {LR}")
    print("=" * 60)
