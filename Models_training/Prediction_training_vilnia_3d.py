...
TRAIN_DATA_FILE = '../vilnia_vilnius_complex_data_2024.csv'  # ← changed
FEATURES_LIST   = ['API_norm', 'S_t', 'SMI_t', 'Pt', 'delta_WL_t']
TARGET          = 'target_change_3d'

MODEL_FILE    = '../ANFIS_models/vilnia_vilnius_anfis_model_3d.pth'  # ← changed
SCALER_X_FILE = '../Scalers/vilnia_vilnius_scaler_X_3d.pkl'  # ← changed
SCALER_Y_FILE = '../Scalers/vilnia_vilnius_scaler_y_3d.pkl'  # ← changed
CONFIG_FILE   = '../Training_configs/vilnia_vilnius_training_config_3d.json'  # ← changed


def prepare_complex_features(df: pd.DataFrame) -> pd.DataFrame:
    df = df.copy().sort_index()

    df['precip_vilniaus-ams'] = df['precip_vilniaus-ams'].clip(lower=0)   # ← changed
    df['water_level_cm']      = df['water_level_cm'].clip(lower=0)

    df['Pt'] = df['precip_vilniaus-ams']                                   # ← changed
    ...
    df['SMI_t'] = df['temp_vilniaus-ams'].apply(                          # ← changed
...
def build_anfis(num_inputs: int, num_mfs: int) -> AnfisNet:
    ...
    return AnfisNet('Vilnia Vilnius Flood Model 3D', invardefs, ['y'], hybrid=True)  # ← changed
...
def train_and_save():
    print("=" * 60)
    print("   🌊 VILNIA–VILNIUS ANFIS 3-DAY TRAINING PIPELINE")     # ← changed
    print(f"   Input  : {TRAIN_DATA_FILE}")
    print(f"   Model  : {MODEL_FILE}")
    print(f"   Station: vilniaus-ams / vilniaus-vilnia-vms")          # ← changed
    print(f"   LR     : {LR}")
    print("=" * 60)
