...
MODEL_FILE    = '../ANFIS_models/marios_birstonas_anfis_model_3d.pth'  # ← changed
SCALER_X_FILE = '../Scalers/marios_birstonas_scaler_X_3d.pkl'  # ← changed
SCALER_Y_FILE = '../Scalers/marios_birstonas_scaler_y_3d.pkl'  # ← changed
CONFIG_FILE   = '../Training_configs/marios_birstonas_training_config_3d.json'  # ← changed
...
def build_anfis(num_inputs: int, num_mfs: int) -> AnfisNet:
    ...
    return AnfisNet('Marios Birstonas Flood Model 3D', invardefs, ['y'], hybrid=True)  # ← changed
...
def train_and_save():
    print("=" * 60)
    print("   🌊 MARIOS–BIRŠTONAS ANFIS 3-DAY TRAINING PIPELINE")  # ← changed
    print(f"   Input  : {TRAIN_DATA_FILE}")
    print(f"   Model  : {MODEL_FILE}")
    print(f"   Station: birstono-ams / birstono-vms")               # ← changed
    print(f"   LR     : {LR}")
    print("=" * 60)
