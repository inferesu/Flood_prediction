...
TRAIN_DATA_FILE = '../nemunas_darsuniskis_complex_data_2024.csv'
FEATURES_LIST   = ['API_norm', 'S_t', 'SMI_t', 'Pt', 'delta_WL_t']
TARGET          = 'target_change'

MODEL_FILE    = '../ANFIS_models/nemunas_darsuniskis_anfis_model.pth'
SCALER_X_FILE = '../Scalers/nemunas_darsuniskis_scaler_X.pkl'  # ← fixed (removed "anfis_model_")
SCALER_Y_FILE = '../Scalers/nemunas_darsuniskis_scaler_y.pkl'  # ← fixed
CONFIG_FILE   = '../Training_configs/nemunas_darsuniskis_training_config.json'  # ← fixed
...
def prepare_complex_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df['precip_kauno-ams'] = df['precip_kauno-ams'].clip(lower=0)   # ← fixed
    df['water_level_cm']   = df['water_level_cm'].clip(lower=0)

    df['Pt'] = df['precip_kauno-ams']                                # ← fixed
    ...
    df['SMI_t'] = df['temp_kauno-ams'].apply(                       # ← fixed
...
def build_anfis(num_inputs: int, num_mfs: int) -> AnfisNet:
    ...
    return AnfisNet('Nemunas Darsuniskis Flood Model', invardefs, ['y'], hybrid=True)  # ← fixed
...
def train_and_save():
    print("=" * 60)
    print("   🌊 NEMUNAS–DARSŪNIŠKIS ANFIS TRAINING PIPELINE")     # ← fixed
    print(f"   Input  : {TRAIN_DATA_FILE}")
    print(f"   Model  : {MODEL_FILE}")
    print(f"   Station: kauno-ams / darsuniskio-vms")               # ← fixed
    print(f"   LR     : {LR}")
    print("=" * 60)
