...
TRAIN_DATA_FILE = '../marios_birstonas_complex_data_2024.csv'  # ← changed
FEATURES_LIST   = ['API_norm', 'S_t', 'SMI_t', 'Pt', 'delta_WL_t']
TARGET          = 'target_change_5d'                          # ← changed

MODEL_FILE    = '../ANFIS_models/marios_birstonas_anfis_model_5d.pth'  # ← changed
SCALER_X_FILE = '../Scalers/marios_birstonas_scaler_X_5d.pkl'  # ← changed
SCALER_Y_FILE = '../Scalers/marios_birstonas_scaler_y_5d.pkl'  # ← changed
CONFIG_FILE   = '../Training_configs/marios_birstonas_training_config_5d.json'  # ← changed


# ---------------------------------------------------------------------------
# Feature engineering  — single station (birstono-ams)
# ---------------------------------------------------------------------------
def prepare_complex_features(df: pd.DataFrame) -> pd.DataFrame:
    df = df.copy().sort_index()

    df['precip_birstono-ams'] = df['precip_birstono-ams'].clip(lower=0)   # ← changed
    df['water_level_cm']      = df['water_level_cm'].clip(lower=0)

    df['Pt'] = df['precip_birstono-ams']                                   # ← changed
    ...
    df['SMI_t'] = df['temp_birstono-ams'].apply(                           # ← changed
        lambda x: max(0, x * 2.5) if x > 0 else 0
    )

    df['delta_WL_t']       = df['water_level_cm'].diff().fillna(0)
    df['target_change_5d'] = df['water_level_cm'].shift(-5) - df['water_level_cm']  # ← changed
...
def build_anfis(num_inputs: int, num_mfs: int) -> AnfisNet:
    ...
    return AnfisNet('Marios Birstonas Flood Model 5D', invardefs, ['y'], hybrid=True)  # ← changed
...
def train_and_save():
    print("=" * 60)
    print("   🌊 MARIOS–BIRŠTONAS ANFIS 5-DAY TRAINING PIPELINE")  # ← changed
    print(f"   Input  : {TRAIN_DATA_FILE}")
    print(f"   Model  : {MODEL_FILE}")
    print(f"   Station: birstono-ams only")                          # ← changed
    print(f"   LR     : {LR}")
    print("=" * 60)
    ...
    assert not has_nan_params(model), "NaN in model at init — check BellMembFunc"
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if has_nan_params(model):
            print(f"\n   ⚠️  NaN in parameters at epoch {epoch} — stopping early.")
        ...
        if (epoch + 1) % 20 == 0:
            nan_flag = " ⚠️ NaN!" if has_nan_params(model) else ""
            print(f"   📉 Epoch [{epoch + 1:3d}/{NUM_EPOCHS}]  "
                  f"Loss: {avg_loss:.6f}  |  Best: {best_loss:.6f}{nan_flag}")

    if best_state is not None:
        model.load_state_dict(best_state)
        print(f"\n   ✔️  Best state restored (loss={best_loss:.6f})")

    if has_nan_params(model):
        print("\n   ❌ Model still contains NaN — not saving. Check your data.")
        return
    ...
    if coeff is not None and torch.isnan(coeff).any():
        print("   ⚠️  coeff has NaN — re-running fit_coeff.")
        with torch.no_grad():
            model.fit_coeff(x_t, y_t)
        coeff = model.coeff
        if isinstance(coeff, np.ndarray):
    ...
    print(f"   ✔️  Config saved → {CONFIG_FILE}")
