...
MODEL_FILE    = '../ANFIS_models/lankupiu_minija_anfis_model.pth'  # ← changed
SCALER_X_FILE = '../Scalers/lankupiu_minija_scaler_X.pkl'  # ← changed
SCALER_Y_FILE = '../Scalers/lankupiu_minija_scaler_y.pkl'  # ← changed
CONFIG_FILE   = '../Training_configs/lankupiu_minija_training_config.json'  # ← changed
...
def prepare_complex_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    avg_t = df[['temp_ventes-ams', 'temp_klaipedos-ams']].mean(axis=1)
...
def build_anfis(num_inputs: int, num_mfs: int) -> AnfisNet:
    ...
    return AnfisNet('Lankupiu Minija Flood Model', invardefs, ['y'], hybrid=True)  # ← changed
...
def train_and_save():
    print("=" * 60)
    print("   🌊 LANKUPIAI–MINIJA ANFIS TRAINING PIPELINE")        # ← changed
    print(f"   Input  : {TRAIN_DATA_FILE}")
    print(f"   Model  : {MODEL_FILE}")
    print(f"   Stations: ventes-ams + klaipedos-ams")
    print(f"   LR     : {LR}")
    print("=" * 60)
