...
MODEL_FILE    = '../ANFIS_models/kanalas_lankupiu_anfis_model.pth'  # ← changed
SCALER_X_FILE = '../Scalers/kanalas_lankupiu_scaler_X.pkl'  # ← changed
SCALER_Y_FILE = '../Scalers/kanalas_lankupiu_scaler_y.pkl'  # ← changed
CONFIG_FILE   = '../Training_configs/kanalas_lankupiu_training_config.json'  # ← changed
...
def prepare_complex_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Average precipitation across both meteo stations
    ...
    # Average temperature across both meteo stations
...
def build_anfis(num_inputs: int, num_mfs: int) -> AnfisNet:
    ...
    return AnfisNet('Kanalas Lankupiu Flood Model', invardefs, ['y'], hybrid=True)  # ← changed
...
def train_and_save():
    print("=" * 60)
    print("   🌊 KANALAS LANKUPIŲ ANFIS TRAINING PIPELINE")        # ← changed
