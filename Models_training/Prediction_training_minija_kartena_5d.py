...
TRAIN_DATA_FILE = '../minija_kartena_complex_data_2024.csv'  # ← changed
FEATURES_LIST   = ['API_norm', 'S_t', 'SMI_t', 'Pt', 'delta_WL_t']
TARGET          = 'target_change_5d'

MODEL_FILE    = '../ANFIS_models/kartena_anfis_model_5d.pth'  # ← changed
SCALER_X_FILE = '../Scalers/kartena_scaler_X_5d.pkl'  # ← changed
SCALER_Y_FILE = '../Scalers/kartena_scaler_y_5d.pkl'  # ← changed
CONFIG_FILE   = '../Training_configs/kartena_training_config_5d.json'  # ← changed


# ---------------------------------------------------------------------------
# Feature engineering  — kretingos-ams + vezaiciu-ams
# ---------------------------------------------------------------------------
def prepare_complex_features(df: pd.DataFrame) -> pd.DataFrame:
    df = df.copy().sort_index()

    # Average across both meteo stations
    df['Pt'] = df[['precip_kretingos-ams', 'precip_vezaiciu-ams']].mean(axis=1)   # ← changed
    ...
    avg_t = df[['temp_kretingos-ams', 'temp_vezaiciu-ams']].mean(axis=1)          # ← changed
...
def build_anfis(num_inputs: int, num_mfs: int) -> AnfisNet:
    ...
    return AnfisNet('Kartena Flood Model 5D', invardefs, ['y'], hybrid=True)       # ← changed
...
def train_and_save():
    print("=" * 60)
    print("   🌊 MINIJA–KARTENA ANFIS 5-DAY TRAINING PIPELINE")   # ← changed
    print(f"   Input  : {TRAIN_DATA_FILE}")
    print(f"   Model  : {MODEL_FILE}")
    print(f"   Stations: kretingos-ams + vezaiciu-ams")            # ← changed
    print(f"   LR     : {LR}")
    print("=" * 60)
