...
MODEL_FILE    = '../ANFIS_models/dane_kretinga_anfis_model.pth'  # ← changed
SCALER_X_FILE = '../Scalers/dane_kretinga_scaler_X.pkl'  # ← changed
SCALER_Y_FILE = '../Scalers/dane_kretinga_scaler_y.pkl'  # ← changed
CONFIG_FILE   = '../Training_configs/dane_kretinga_training_config.json'  # ← changed
...
def build_anfis(num_inputs: int, num_mfs: int) -> AnfisNet:
    ...
    return AnfisNet('Dane Kretinga Flood Model', invardefs, ['y'], hybrid=True)  # ← changed
...
def train_and_save():
    print("=" * 60)
    print("   🌊 DANĖ–KRETINGA ANFIS TRAINING PIPELINE")          # ← changed
    print(f"   Input  : {TRAIN_DATA_FILE}")
    print(f"   Model  : {MODEL_FILE}")
    print(f"   Station: kretingos-ams / kretingos-vms")           # ← changed
    print(f"   LR     : {LR}")
    print("=" * 60)
