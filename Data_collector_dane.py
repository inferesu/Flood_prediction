...
K_DECAY = 0.85  # Decay coefficient for API
    ...
    while current <= end_date and current.date() <= today:
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                print(f"    [{obs_type.upper():10}] {date_str}  |  HTTP {resp.status_code} — skipped")
...
def fetch_meteo_data(station_code, start_date, end_date):
    ...
    print(f"\nStarting meteo fetch for station: [{station_code}]")
    ...
    while current <= end_date and current.date() <= datetime.utcnow().date():
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                print(f"    {date_str}  |  P={daily_p:5.2f}mm  T={avg_t:6.2f}°C  |  [{progress_pct:5.1f}%] ({days_done}/{total_days})")
            else:
                print(f"    {date_str}  |  HTTP {resp.status_code} — skipped  |  [{progress_pct:5.1f}%]")
        except Exception as e:
...
def apply_hydrological_logic(df):
    ...
    print(f"\n⚙Applying hydrological logic to {len(df)} rows...")

    df = df.copy().sort_values('timestamp')

    # 1. Pt (Precipitation from single station)
    df['Pt'] = df['precip_klaipedos-ams']
    print(f"   [Eq. 9]  Pt computed — mean={df['Pt'].mean():.2f}mm, max={df['Pt'].max():.2f}mm")
    ...
    df['API_t'] = api_vals
    print(f"    [Eq. 9]  API_t computed — k={K_DECAY}, mean={df['API_t'].mean():.2f}, max={df['API_t'].max():.2f}")

    # 3. SMI_t (Snowmelt Index) (Eq. 17)
    df['SMI_t'] = df['temp_klaipedos-ams'].apply(lambda x: max(0, x * 2.5) if x > 0 else 0)
    melt_days = (df['SMI_t'] > 0).sum()
    print(f"    [Eq. 17] SMI_t computed — {melt_days} melt days detected, max={df['SMI_t'].max():.2f}")

    # 4. Season_cos (Eq. 11)
    df['S_t'] = np.cos(2 * np.pi * pd.to_datetime(df['timestamp']).dt.dayofyear / 365)
    print(f"   [Eq. 11] S_t (seasonal cosine) computed — range=[{df['S_t'].min():.2f}, {df['S_t'].max():.2f}]")

    # 5. Trend/State (Eq. 20-21)
    df['delta_WL_t'] = df['water_level_cm'].diff().fillna(0)
    rising = (df['delta_WL_t'] > 0).sum()
    falling = (df['delta_WL_t'] < 0).sum()
    print(f"    [Eq. 20] delta_WL_t computed — rising={rising} days, falling={falling} days")
...
def main():
    ...
    print(f"  Water levels aggregated to {len(df_w)} daily rows")
    ...
    print(f"   Date range in file: {df_final['timestamp'].min()} → {df_final['timestamp'].max()}")
