...
# --- Configuration ---
DATA_FILE = 'druskininkai_complex_data_2024.csv'
STATION_CODE_HYDRO = 'druskininku-vms'
STATION_CODE_METEO_1 = 'druskininku-ams'
...
def apply_hydrological_logic(df):
    ...
    df['Pt'] = df[f'precip_{STATION_CODE_METEO_1}']
    ...
    df['SMI_t'] = df[f'temp_{STATION_CODE_METEO_1}'].apply(lambda x: max(0, x * 2.5) if x > 0 else 0)
    melt_days = (df['SMI_t'] > 0).sum()
    print(f"   ✔️  [Eq. 17] SMI_t computed — {melt_days} melt days detected, max={df['SMI_t'].max():.2f}")

    # 4. Season_cos (Eq. 11)
    df['S_t'] = np.cos(2 * np.pi * pd.to_datetime(df['timestamp']).dt.dayofyear / 365)
    print(f"   ✔️  [Eq. 11] S_t (seasonal cosine) computed — range=[{df['S_t'].min():.2f}, {df['S_t'].max():.2f}]")

    # 5. Trend/State (Eq. 20-21)
    df['delta_WL_t'] = df['water_level_cm'].diff().fillna(0)
    rising = (df['delta_WL_t'] > 0).sum()
    falling = (df['delta_WL_t'] < 0).sum()
    print(f"   ✔️  [Eq. 20] delta_WL_t computed — rising={rising} days, falling={falling} days")
...
def main():
    ...
    print("=" * 60)
    print("   🌊 DRUSKININKAI HYDROLOGICAL DATA PIPELINE")
